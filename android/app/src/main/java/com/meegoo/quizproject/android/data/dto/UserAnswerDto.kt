...
sealed class UserAnswerDto {
    class StringAnswer(var answer: String) : UserAnswerDto()
    class ChoiceAnswer(val answer: MutableList<UUID>) : UserAnswerDto()
    ...
    companion object {
        fun parseFromEntity(data: Any): UserAnswerDto {
            return if (data is List<*>) {
                ChoiceAnswer(data.map { UUID.fromString(it.toString()) }.toMutableList())
            } else {
